...
- Documents flow: quotes/invoices/receipts share one `documents` table (`type` column: `quote`|`invoice`|`receipt`; `lineItems` stored as a JSON string, parsed/serialized via `serializeDocument()` in `server.js`). Numbers are sequential per type per year (`generateDocumentNumber()`, e.g. `INV-2026-0001`) — never assign numbers client-side. `src/pages/AdminDocuments.tsx` (list) and `AdminDocumentForm.tsx` (create) are JWT-protected; `GET /api/admin/documents/:id/pdf` renders a branded PDF (logo, KRA PIN, QR code) via `pdfkit` + `qrcode`. The QR encodes `${CLIENT_URL}/verify/{number}`, resolved by `src/pages/VerifyDocument.tsx` — a public, unauthenticated page/endpoint (`GET /api/verify/:number`) by design, since that's what lets anyone holding a printed/emailed document confirm it's genuine.
